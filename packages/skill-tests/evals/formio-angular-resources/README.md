...
Conceptually: edit SKILL.md → rerun evals → grade → compare → decide. The `skill-creator` skill orchestrates this for you if you prefer (the agent handles spawning subagents, timing, grading, and viewer launch). The rest of this README documents the manual flow for reference.
...
cp -R packages/skill-tests/evals/formio-angular-resources/fixtures/existing-workspace-seed \
...
From a client that can spawn subagents — these benchmarks were run under Claude Code, and this is what `skill-creator` does on your behalf:

- **With-skill run** for each of the three evals: spawn a `general-purpose` subagent, tell it to read `plugin/skills/formio-angular/formio-angular-resources/SKILL.md`, hand it the eval prompt from `evals.json`, instruct it to write output files under `$OUT/eval-N-<name>/with_skill/outputs/workspace/` and the Phase A plan as `outputs/phase-a-plan.md`. Override any `/tmp/*` paths in the prompt — they're literal in `evals.json` so humans can read them, but your run re-targets them to the iteration directory.
...
```bash
ITERATION=iteration-$N python3 packages/skill-tests/evals/formio-angular-resources/grade.py
```
