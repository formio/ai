...
From a client that can spawn subagents — these benchmarks were run under Claude Code, and `skill-creator` handles the orchestration for you if it is installed. Fall back to this manual flow otherwise:

- **With-skill run** for each eval: spawn a `general-purpose` subagent, have it read `plugin/skills/formio-resource-planner/SKILL.md`, hand it the eval prompt from `evals.json`, and write Phase B outputs (`template.md` and `template.json`) to `$OUT/<eval>/with_skill/outputs/`.
...
```bash
ITERATION=iteration-$N python3 packages/skill-tests/evals/formio-resource-planner/grade.py
```
