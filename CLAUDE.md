...
The library's default "build me an app" entry point is [`plugin/skills/formio-application/`](plugin/skills/formio-application/) — a framework-agnostic orchestrator that runs the full pipeline from plain-language intent through `formio-resource-planner`, the `project_import` MCP tool, and handoff to a framework-specific scaffolding skill. There are two framework implementors. [`plugin/skills/formio-angular/`](plugin/skills/formio-angular/) (with its sub-skill [`plugin/skills/formio-angular/formio-angular-resources/`](plugin/skills/formio-angular/formio-angular-resources/) for per-resource NgModule work) is the Angular one, and the registry's default. [`plugin/skills/formio-react/`](plugin/skills/formio-react/) is the React one: a **router over branches** rather than one linear flow — greenfield build, add-CRUD-to-an-existing-app, and embed a single form — delegating per-resource work to [`plugin/skills/formio-react/formio-react-resources/`](plugin/skills/formio-react/formio-react-resources/) and single-form embedding to [`plugin/skills/formio-react/formio-react-form/`](plugin/skills/formio-react/formio-react-form/). Neither is a top-level build-an-app skill; both are implementors `formio-application` delegates to, and with two rows in the registry a greenfield build now asks which framework, defaulting to Angular when the user declines to choose. Further framework skills add themselves as rows in `plugin/skills/formio-application/FRAMEWORK.md`'s registry table; no other change to the orchestrator is required.

The two implementors are deliberately NOT parallel in design. `formio-angular` configures `@formio/angular`'s `FormioResource` module, which the library ships. `@formio/react` has no equivalent, so `formio-react` GENERATES a small resource kernel into the user's application — pure domain functions plus React Router loader/action factories — rather than porting Angular's service, registry, alert bus, and refresh emitter, all of which are scaffolding for capabilities React Router already provides. The generated app is Vite + React Router **data routers**, client-rendered; server rendering and Next.js App Router are out of scope.

Embedding splits along the same seam. `formio-form` stays the framework-agnostic entry point and the single owner of definition-level behaviour — conditionals, calculated values, validation, cascading selects — which every framework embed skill links to rather than restating. What differs per framework is only the **mounting**: `formio-react-form` covers `@formio/react`'s `Form` component, its lifecycle traps, provider configuration, anonymous versus authenticated submission, and the Vite and Next.js requirements. `formio-form` checks the host before writing mounting code and hands a React workspace over; the equivalent Angular embedding skill does not exist yet, and `formio-form` says so on contact rather than presenting Vanilla JS as the recommended Angular path.
...
Harnesses that exist today: `packages/skill-tests/evals/formio-resource-planner/`, `packages/skill-tests/evals/formio-angular-resources/`, and `packages/skill-tests/evals/formio-react-resources/` (the latter two grade the nested sub-skills at `plugin/skills/formio-angular/formio-angular-resources/` and `plugin/skills/formio-react/formio-react-resources/`). The consolidated `formio-api` skill has no harness — add one at `packages/skill-tests/evals/formio-api/` following the same layout if you need to measure reference-navigation or activation-precision changes there.
