...
### POST {baseUrl}/formio/user/login
...
curl -D - -X POST -H "Content-Type: application/json" \
  ...
  "{baseUrl}/formio/user/login"
```

### POST {baseUrl}/formio/user
...
### POST {baseUrl}/formio/user/login (as a platform user)
...
```bash
curl -H "x-jwt-token: $FORMIO_JWT" "{baseUrl}/current"
```

### GET {baseUrl}/formio/logout
...
```bash
curl -H "x-jwt-token: $FORMIO_JWT" "{baseUrl}/formio/logout"
```
