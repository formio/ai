...
The Postman collection composes the project endpoint as `{{baseUrl}}/{{projectName}}`, but **never build a project URL that way yourself** — only one of the deployment routings puts the project on a path under the base URL, so composing it that way is wrong for the others. Read both values from the MCP server instead, with `npx -y @formio/mcp@0.11.0 project get --cwd "$(pwd)"`, and use exactly what it prints.
