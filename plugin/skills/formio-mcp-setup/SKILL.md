...
```bash
npx -y @formio/mcp@0.12.2 project set --project-url "<project url>" --base-url "<base url>" --cwd "$(pwd)"
```
...
1. **Global install from an internal registry or a cached tarball**, then point the configuration at the binary instead of `npx`:
   ...
   ```bash
   npm install -g @formio/mcp@0.12.2
   ```

   Replace `"command": "npx", "args": ["-y", "@formio/mcp@0.12.2"]` with `"command": "formio-mcp", "args": []` in every file you write (and the TOML equivalent).
