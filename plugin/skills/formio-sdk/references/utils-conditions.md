...
- `Utils.checkCustomConditional(component, custom, row, data, form, variable, onError, instance): boolean` — evaluate a JS string and return the value of `variable` (commonly `'show'` or `'result'`). When `custom` is a string, the SDK wraps it as `var ${variable} = true; ${custom}; return ${variable};`. The `custom` string is compiled and run, so it comes from a form definition in the application's own project or a literal in its source — never from a submission or a request; prefer `checkJsonConditional`, which evaluates data rather than code, whenever the rule can be expressed as JSONLogic.
