...
**Page layout is NOT yours to set.** The app shell — `src/app/app.html` (legacy `app.component.html`), specified in the parent skill's `AUTH.md` → "Page layout contract" — wraps `<router-outlet>` in a single page-layout element that owns horizontal gutters, max content width, and top spacing for every route. These templates start INSIDE those gutters. Do not add a page-level layout wrapper (a container / max-width / page-padding element) at the top of `resource.component.html` or `view/view.component.html`: only two templates per resource are overridden, so the wrapper would double-pad the pages you author while `/<resource>/new`, `/<resource>/:id/edit`, `/<resource>/:id/delete`, `/<resource>` (unless the index is overridden too), `/auth/login`, and `/auth/register` — all library components you cannot wrap — stay unpadded. That asymmetry hides the bug rather than fixing it: the pages you wrote look right and the rest do not. Layout that is internal to a page (grid rows, columns, cards, gaps) is yours; the page's outer boundary is the shell's. This holds in every design language.
