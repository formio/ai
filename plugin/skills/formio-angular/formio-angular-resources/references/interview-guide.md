...
2. **Form.io project URL** (`projectUrl`) — the value that goes into `FormioAppConfig.appUrl`. Not a question to ask when the Form.io MCP tools are callable by you: resolve it by calling `project_get` with `cwd` set to the workspace root, and reconcile against `src/app/config.ts`, asking only for whichever value that report names as missing. With no Form.io tools at all it IS a question — ask the user for the Project URL alone and derive the Base URL from it, per [`project-urls.md`](../../../formio-mcp-setup/references/project-urls.md). Do not install the MCP server to answer it.
