...
- **Do not hand-roll the Angular workspace.** When the working directory does not yet contain one, BOOTSTRAP offers to install the Angular team's official `angular-new-app` skill — that one skill, by name, from `angular/skills` — and delegates to it; if the user declines that install, it falls back to the Angular CLI (`npx @angular/cli@<major> new`) under its own approval. Both paths are in [`BOOTSTRAP.md`](./BOOTSTRAP.md) — never hand-write an `angular.json` / `package.json`, and never run a scaffolding command the user has not approved.
...
- Empty `workspaceRoot`, planner pair in hand: "`<workspaceRoot>` is empty — I'll confirm which Form.io project this directory is configured for (SETUP), then install the Angular team's `angular-new-app` skill and delegate to it to scaffold the workspace there (BOOTSTRAP), then wire Form.io into it (CONFIG, AUTH, Resources)." Name the absolute path rather than "this folder", so a wrong root is caught before anything is scaffolded into it.
...
**Goal:** when the working folder does not yet contain an Angular workspace, install the Angular team's official `angular-new-app` skill — that one skill, by name, from their `angular/skills` repository — and delegate workspace creation to it. Then install `@formio/angular` + `@formio/js` so CONFIG can import from them; install Bootstrap 5 + Bootstrap Icons and register their stylesheets in `angular.json` (unless the user explicitly opted out — see `BOOTSTRAP.md` Step 4); and **pin zoneless change detection explicitly** (`provideZonelessChangeDetection()`, no `zone.js` in `polyfills`) so generated apps are deterministic — see `BOOTSTRAP.md` Step 5 for the rationale and the Form.io-specific caveat.

**How:** read [`BOOTSTRAP.md`](./BOOTSTRAP.md) for the version-resolution steps, the exact `npx skills add … --skill angular-new-app` invocation and the `--list` preview that precedes it, the handoff shape to `angular-new-app`, the post-scaffold verification checks, the SDK/Bootstrap install steps, and the skip-if-already-scaffolded detection logic.
...
- https://github.com/angular/skills — Angular team's official skill repository (BOOTSTRAP offers to install its `angular-new-app` skill, and only that one; the user decides)
