...
- **`[options]`** configures the **component**: `errors.message`, `alerts.submitMessage`, `disableAlerts`, `alertsPosition`, and `hooks.beforeSubmit`. Of the renderer's own options it forwards only a whitelist — `noAlerts`, `i18n`, `fileService`. Anything else you put here is **silently dropped** before the renderer sees it. **Use `disableAlerts: true` to turn the alerts off, not `alertsPosition`.** The component compares `alertsPosition` against a numeric `AlertsPosition` enum by identity, so a string value matches nothing and silently renders no alerts — and the enum itself is **not exported** from `@formio/angular` (it is declared in the type surface but absent from the package's export lists), so there is no supported way to name a position from application code. `initialize()` maps `disableAlerts: true` to `AlertsPosition.none` internally, which is the one route that works. To place alerts somewhere else, switch them off and render your own from `(errorChange)`.
