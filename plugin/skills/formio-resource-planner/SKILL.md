...
- **Don't call the MCP server.** The skill produces plans plus the `template.md` / `template.json` artifact pair. It does not call `project_import`, `form_create`, or any other MCP tool, which is why it needs no project configuration of its own and probes for none. Importing is a separate, explicit user action: Phase B prints it as a next step, and whoever carries it out — `formio-application` on its own flow, or you in a later turn once the user asks — confirms the target project first, on the terms [`references/phase-b-emission.md`](references/phase-b-emission.md) → "Next steps" sets out.
