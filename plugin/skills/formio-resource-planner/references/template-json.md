...
**When `data.<parentKey>.data` is reachable at all.** In the browser, picking a parent in a `dataSrc: resource` select puts that row's full submission into `data.<parentKey>`, so `.data.<groupKey>` resolves and the mirror is populated at selection time. On read, the server expands a `reference: true` value into the full submission only when the caller has `read_all` on the referenced resource, owns the row, or is an administrator (`checkReferenceReadAccess`) — group membership does NOT qualify, so for exactly the group-scoped users this pattern exists for, a stored grandchild comes back with `data.<parentKey>` as a bare `{ _id }` and the optional chain is the ONLY path they ever take. The mirror also uses `reference: true` — it stores the same kind of resolvable reference as the original.

**What the fallback costs.** `|| value` keeps the mirror's previous team whenever the parent yields none — not only on the unexpanded-`{ _id }` load it exists for, but also when the user clears the parent select or re-parents the row onto an account whose row carries no `<groupKey>` (one created by import or API without a group). In those cases the row keeps the OLD group's ACL. `validate.required: true` blocks a save with no parent, and re-parenting onto an account that does carry a group overwrites correctly, so the exposure is the group-less parent; if the app allows such parents, add a `customConditional` or server-side check rather than weakening the guard. Note that the calculation runs in the browser only — `calculateComponentValue` returns early on the server unless `calculateServer: true`, and a server-side run would see the unexpanded `{ _id }` anyway — so the mirror is a renderer-side convenience the field-based `submissionAccess` then enforces, not a server-computed invariant.
