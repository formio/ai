...
4. **Join resources with group-based access.** Every join resource that implements group-level access (e.g., `ProjectUser`, `TeamUser`, `CompanyUser`) needs a Group Assignment action keyed `"<join>:group"`. The action's `settings.group` MUST match the field key on the join whose `select` points at the group resource; `settings.user` MUST match the field key whose `select` points at the user resource. A join without Group Assignment stores rows but never grants access — the whole group-access model is dead. Two companion requirements: every child whose access flows from that group carries the field-based block on its group-reference `select`, with types chosen deliberately (the four-entry form confers delete on every group member); and the **group resource itself** gets `read_all` for the end-user role, because nothing stamps a group row's own `access` and the group must be readable to populate the select whose value authorizes group-based create. Do NOT add `create_own` to a group-scoped child — the block already authorizes create, and `create_own` would additionally allow creating rows outside the group. See [`template-json.md`](template-json.md) → "Group Assignment (Group Permissions)" and → "Choosing the types", and [`template-md.md`](template-md.md) → "Token → `template.json` mapping".
