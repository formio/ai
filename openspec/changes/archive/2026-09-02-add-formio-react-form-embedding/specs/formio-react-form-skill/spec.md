...
- **Source precedence.** `form` (a definition object) takes precedence over `src` (a URL string) when both are supplied; `url` sets the instance URL when the source is an object. Passing a definition means the application owns fetching; passing a URL means the renderer fetches.
...
- **Custom form classes.** `FormClass` supplies an alternate constructor — a Wizard or PDF renderer in place of the default. The older `formioform` prop is its deprecated alias.
...
- **THEN** it states that `form` wins over `src` when both are passed
...
The renderer emits Bootstrap-classed markup and ships no stylesheet of its own. An application that renders a form without supplying CSS gets an unstyled, visually broken form — the first thing a reader hits and the symptom most likely to be misread as a rendering failure.

`styling.md` SHALL cover:

- That a stylesheet is required, and the options: importing Bootstrap, supplying the application's own CSS against the renderer's class names, or a template framework.
...
- The symptom-first framing: "the form renders but looks broken" means no stylesheet, not a mis-mounted component.
...
- **THEN** it states that the renderer ships no stylesheet and one must be supplied
...
### Requirement: Deprecated surfaces are marked, never recommended

The sub-skill SHALL identify the surfaces a reader will encounter in existing code or older examples and SHALL NOT present any of them as the way to do new work:

- **`ReactComponent`** — the base class for custom React components inside the renderer. It is deprecated, warns at construction and initialization, and its own guidance is to write a custom `@formio/js` component instead. The sub-skill SHALL document it only as something to migrate away from.

  A deprecation notice with no destination is worse than none, so the sub-skill SHALL document the replacement path concretely: a custom component extends a `@formio/js` component class and is registered with `Formio.Components.addComponent`, and `Components` is re-exported from `@formio/react` so the registration needs no second import. It SHALL show where that registration goes in a React application — once at module scope, before any form renders, not inside a component body — and SHALL route the component-class API itself to `formio-sdk`.
- **The legacy Redux modules** — `auth`, `form`, `forms`, `submission`, `submissions`. They predate the current surface and are not wired to `FormioProvider`. The sub-skill SHALL NOT use them and SHALL warn that the singular and plural modules are different slices — entity CRUD versus list state — that read almost identically and fail silently when confused.
- **The prop aliases** `formioform` and `formReady`.

#### Scenario: ReactComponent is documented as a migration target

- **WHEN** the references mention `ReactComponent`
- **THEN** they mark it deprecated and direct new work to a custom `@formio/js` component
