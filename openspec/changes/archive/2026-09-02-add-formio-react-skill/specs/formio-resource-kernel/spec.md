...
The Form.io-specific behavior the kernel owns SHALL be expressed as pure, synchronous functions that take data and return data, with no fetching, no React, and no router imports:
...
- `parentFilters({ parents, params })` → a submission-query object. For each parent entry not marked `filter: false`, produces `data.<path>._id` equal to the parent id read from route params.
- `resourcePermissions({ user, form, submission })` → `{ create, read, edit, delete }`, wrapping `Formio.userPermissions`.
...
- **AND** it does not read React context and does not depend on a previously-set SDK global
...
### Requirement: The current user is bindable as an ancestor

A binding MAY name the current user instead of a resource config, as `{ resource: 'currentUser', field: 'user', filter? }`. It SHALL pre-fill the named component with the user loaded by the root loader, hiding it the same way, and — when filtering is enabled — narrow the list to that user's records.

This is the React equivalent of the `currentUser` entry Angular seeds into its registry from `FormioAuthService.userReady`. It carries the same two uses: `filter: false` to stamp the author onto new records, and filtering enabled for a "my records" list. Client-side filtering is a convenience, never an access control boundary; the deployment enforces access server-side.

#### Scenario: Author stamped without narrowing the list

- **WHEN** a binding is `{ resource: 'currentUser', field: 'user', filter: false }`
- **THEN** new submissions carry the current user in the `user` component
- **AND** the list is not narrowed by it

#### Scenario: My-records list narrows by the current user

- **WHEN** the same binding enables filtering
- **THEN** the list query carries the current user's id at the component's resolved path
...
The kernel SHALL export `requireUser(loader)`, which wraps a loader so that an unauthenticated request returns `redirect('/login')` before the wrapped loader runs. Because the check happens during navigation, no protected screen mounts for an unauthenticated visitor and there is no intermediate render to suppress.
