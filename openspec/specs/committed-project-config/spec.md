# committed-project-config Specification

## Purpose

Defines `formio.json`: the committed counterpart to the machine-local project map. It records which Form.io project a directory targets in a file tracked by version control, discovered by walking up from the caller's working directory, so the target survives a clone and is visible in review.
