...
Defines the browser-based portal-login flow the server uses to obtain a JWT: an ephemeral local login server, the rendered project login form, the callback that captures the token, and the shutdown that follows.
## Requirements
