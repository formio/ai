...
**The check names one concrete tool.** It used to ask, abstractly, whether "the Form.io tools" were callable, and abstraction invites an agent to reason about whether they are present but not yet usable. `form_list` admits no such reading, and everything written to argue against it is gone.
